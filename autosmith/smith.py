from .types import ToolEnv, Function
from typing import Optional
from .env import get_requirements
import tempfile
import os
from .template import render_container, render_server, render_requirements
import subprocess
import time
import urllib.request

def smith(func: Function, 
          env: Optional[ToolEnv] = None) -> ToolEnv:
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        output = subprocess.run(["docker", "ps", "-a", "-q", "--filter", f"ancestor={env.title}"], capture_output=True)
        ...
        output = subprocess.run(["docker", "run", "-d", "-p", f"{env.port}:8080", env.title], capture_output=True)
        if output.returncode != 0:
